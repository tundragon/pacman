...
const INVALID_ARGS_ERROR = "[rollup-plugin-generate-html-template] You did not provide a template or target!";
/**
  ...
  return {
    ...
    async generateBundle(outputOptions, bundleInfo) {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          if (!target && !template) throw new Error(INVALID_ARGS_ERROR); // Get the target file name.

          const targetName = path.basename(target || template); // Add the file suffix if it isn't there.

          const targetFile = targetName.indexOf(".html") < 0 ? `${targetName}.html` : targetName; // Read the file
          ...
          const injected = [tmpl.slice(0, bodyCloseTag), ...bundles.map(b => `<script ${scriptTagAttributes.join(" ")} src="${prefix || ""}${b}"></script>\n`), tmpl.slice(bodyCloseTag, tmpl.length)].join(""); // write the injected template to a file
